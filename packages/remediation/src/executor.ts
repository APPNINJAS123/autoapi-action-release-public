...
function buildPrompt(
  ...
  return [
    ...
    ...(files.some(file => file.path.endsWith('.go')) ? [
      ...
      'Compare every exact target Go constructor return type with each supplied caller, callee parameter, and stored field that consumes it. A change from *T to T or T to *T must be reconciled coherently at that boundary using only ordinary Go address/value semantics and the supplied declarations; do not wait for a later compiler attempt to reveal the mismatch.',
